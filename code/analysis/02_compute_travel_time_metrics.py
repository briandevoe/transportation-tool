...
DEST_DIR = REPO_ROOT / "data" / "layer4_destination" / "schools" / "processed"
...
def load_origins(origin_source, state_fips, year):
    ...
    if not matches:
        raise SystemExit(f"No origins file found for --origin-source {origin_source}, state {state_fips}, year {year}")
...
def build_graph(state_fips):
    ...
    if not edges_path.exists() or not nodes_path.exists():
        raise SystemExit(f"Missing layer2 OSM network for state FIPS {state_fips} in {NETWORK_DIR} (run layer2 first)")
...
def main():
    ...
    parser.add_argument("--dest-year", default="2425", help="Destinations file vintage (default: 2425)")
    ...
    for dest_type, filename in [
        ("school_highschool", f"highschools_{args.dest_year}.parquet"),
        ("school_elementary_middle", f"elementary_middle_{args.dest_year}.parquet"),
    ]:
        dest_path = DEST_DIR / filename
        if not dest_path.exists():
            raise SystemExit(f"Missing destinations file: {dest_path}")
        dests = pd.read_parquet(dest_path)
    ...
    out_path = OUT_DIR / state_fips / f"travel_time_metrics_{args.dest_year}.parquet"
